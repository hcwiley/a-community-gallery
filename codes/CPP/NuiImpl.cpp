...
void CSkeletalViewerApp::myInit(){
	...
	for(int j = 0; j < numGalleries; j++){
		for(int i = 0; i < numImages; i++){
			...
			cvReleaseImage(&tmp);
			images[j][i]->x(300+(i*(IMG_WIDTH + 100)));
			images[j][i]->y(40);
			...
			images[j][i]->galNum = j;
			int length;
			sprintf(imagePath,"..\\camera%d\\%d-title.txt", j+1, i+1);
			ifstream is;
			is.open (imagePath);
			is.seekg (0, ios::end);
			length = is.tellg();
			images[j][i]->title = new char [length];
			is.seekg (0, ios::beg);
			is.read(images[j][i]->title, length);
			is.close();
		}
	}
	gallery = 0;
...
void CSkeletalViewerApp::copyImage(IMAGE* dst, IMAGE* src){
	...
	/*
	ifstream is;
	sprintf(imagePath,"..\\camera%d\\%d-artist.txt", j+1, i+1);
	is.open (imagePath);
	is.seekg (0, ios::end);
	length = is.tellg();
	is.seekg (0, ios::beg);
	...
	sprintf(imagePath,"..\\camera%d\\%d-year.txt", j+1, i+1);
	is.open (imagePath);
	is.seekg (0, ios::end);
	length = is.tellg();
	is.seekg (0, ios::beg);
	...
	is.close();
	sprintf(imagePath,"..\\camera%d\\%d-materials.txt", j+1, i+1);
	is.open (imagePath);
	is.seekg (0, ios::end);
	length = is.tellg();
	is.seekg (0, ios::beg);
...
void CSkeletalViewerApp::watchSkeleton(){
	...
	if(initDone){
		...
		if( person.distance < thresh1){
			cvRectangle(bgImg,cvPoint(400,HELP_TEXT_Y - 200),cvPoint(WIDTH - 400,HELP_TEXT_Y + 200), cvScalar(255,255,255), CV_FILLED);
